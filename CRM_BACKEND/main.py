...
try:
    ...
    """)
    print("Notifications and user_devices tables verified/created.")
...
def notify_user(user_id: int, title: str, message: str, type_str: str):
    try:
        ...
        if user and user["email"]:
            ...
            threading.Thread(target=send_email, args=(user["email"], title, email_body), daemon=True).start()
            ...
            if admins:
                for admin in admins:
                    ...
                    if admin["email"]:
                        ...
                        """
                        from mail import send_email
                        threading.Thread(target=send_email, args=(admin["email"], f"[Admin Oversight] {title}", admin_email_body), daemon=True).start()
    except Exception as e:
...
def login(req: LoginRequest):
    ...
    if not existing:
        ...
        notify_user(
            ...
            message=f"{user['name']} has checked in successfully at {now.strftime('%I:%M %p')}.",
            type_str="attendance"
        )
    else:
...
def verify_user(employee_id: str, current_user: dict = Depends(RoleChecker(["admin", "hr"]))):
    ...
    notify_user(
        ...
        message=f"Congratulations {user['name']}, your Papyrus CRM account has been verified by the administrator and is now fully active.",
...
def check_out(req: LocationUpdate, employee_id: str = Query(...), current_user: dict = Depends(get_current_user)):
    ...
    notify_user(
        ...
        message=f"{user['name']} has checked out successfully at {now.strftime('%I:%M %p')}{note_str}.",
        type_str="attendance"
    )
...
def pin_location(req: PinLocationRequest, employee_id: str = Query(...), current_user: dict = Depends(get_current_user)):
    ...
    notify_user(
        ...
        message=f"{user['name']} pinned location: '{req.note}' (Coords: {req.lat}, {req.lon}).",
        type_str="location"
    )
...
def create_lead(req: LeadCreate, current_user: dict = Depends(get_current_user)):
    ...
    notify_user(
        user_id=uid,
        title="Lead Created",
        message=f"Lead for {cust_display} was successfully created by {current_user['name']}.",
    ...
    if assigned_sales_id:
        notify_user(
            ...
            message=f"You have been assigned to lead: {cust_display}.",
        ...
        if req.followup_date:
            notify_user(
                ...
                message=f"A follow-up has been scheduled for lead {cust_display} on {req.followup_date}.",
                type_str="followup"
            )
        ...
        if admins:
            for admin in admins:
                notify_user(
                    ...
                    message=f"A new lead for {cust_display} was created by {current_user['name']} and is pending verification.",
...
def update_lead(id: int, req: LeadUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if assigned_sales_id and assigned_sales_id != lead["assigned_sales_id"]:
        notify_user(
            ...
            message=f"Lead for {cust_display} has been reassigned to you.",
            type_str="lead_assignment"
        )
        if req.followup_date:
            notify_user(
                user_id=assigned_sales_id,
                title="Lead Follow-up Scheduled",
                message=f"A follow-up is scheduled for lead {cust_display} on {req.followup_date}.",
                type_str="followup"
            )
        ...
        notify_user(
            ...
            message=f"Follow-up date for lead {cust_display} has been updated to {req.followup_date}.",
            type_str="followup"
        )
...
def verify_lead(id: int, current_user: dict = Depends(RoleChecker(["admin", "hr"]))):
    ...
    if lead.get("assigned_sales_id"):
        ...
        notify_user(
            ...
            message=f"Lead for {cust_display} has been approved and is now active ({new_status}).",
...
def deduct_and_lock_inventory(cur, order_id: int, customer_id: int, product_name: str, quantity: float):
    ...
    if inv_row:
        ...
        cur.execute(
            """
            INSERT INTO inventory (product_id, category, current_stock, minimum_stock, unit)
            VALUES (%s, 'Indents', %s, 100, 'Units') RETURNING id
            """,
    ...
    if inv_check:
        ...
        if curr_stock <= min_stock:
            msg = f"Stock level for {product_name} is low: {curr_stock} (Minimum required: {min_stock})."
            cur.execute(
                """
                SELECT u.id, u.email FROM users u 
                JOIN roles r ON u.role_id = r.id 
            ...
            for u_id, u_email in users_to_notify:
                cur.execute(
                    ...
                    (u_id, "Low Stock Alert", msg, "stock")
                )
                if u_email:
                    email_body = f"""
                    <html>
...
def convert_lead_to_deal(id: int, req: LeadConvertRequest, current_user: dict = Depends(get_current_user)):
    ...
    msg = f"Lead for {cust_display} has been converted to Order {order_number} (Value: ₹{total_val})."
...
def create_deal(req: DealCreate, current_user: dict = Depends(get_current_user)):
    ...
    msg = f"New Order {order_number} has been created for {cname} (Value: ₹{total_val})."
...
def update_deal(id: int, req: DealUpdate, stage: Optional[str] = None, status: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    ...
    if db_status != order["status"]:
        ...
        msg = f"Order {order['order_number']} ({cname}) status updated to: {final_stage} | {final_status}."
...
def approve_deal(id: int, current_user: dict = Depends(RoleChecker(["admin"]))):
    ...
    msg = f"Order {order['order_number']} for {cname} has been APPROVED by Admin."
...
def reject_deal(id: int, current_user: dict = Depends(RoleChecker(["admin"]))):
    ...
    cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
    cname = cust["company_name"] if cust else "Customer"
    msg = f"Order {order['order_number']} for {cname} has been REJECTED by Admin."
...
def update_inventory_item(id: int, req: InventoryItemUpdate, current_user: dict = Depends(RoleChecker(["admin", "inventory"]))):
    ...
    if req.current_stock <= req.minimum_stock:
        msg = f"Stock level for {req.item_name} is low: {req.current_stock} (Minimum required: {req.minimum_stock})."
    ...
    if admins_accs:
        for user in admins_accs:
            notify_user(
                ...
                message=f"A purchase request for {req.quantity} units of {req.item_name} has been raised by {current_user['name']} and is pending approval.",
...
def approve_purchase_request(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    notify_user(
        ...
        message=f"Your purchase request for {pr['quantity']} units of {pr['item_name']} has been approved by {current_user['name']}.",
        type_str="purchase_request"
    )
...
def reject_purchase_request(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    notify_user(
        ...
        message=f"Your purchase request for {pr['quantity']} units of {pr['item_name']} has been rejected by {current_user['name']}.",
        type_str="purchase_request"
    )
...
def update_production_status(id: int, req: ProductionUpdate, current_user: dict = Depends(RoleChecker(["admin", "production"]))):
    ...
    if order:
        cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
        cname = cust["company_name"] if cust else "Customer"
        msg = f"Production status for Order {order['order_number']} ({cname}) is now {req.status}."
...
def update_invoice_payment_status(id: int, req: InvoicePaymentUpdate, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    if order:
        cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
        cname = cust["company_name"] if cust else "Customer"
        msg = f"Invoice {inv['invoice_number']} for Order {order['order_number']} ({cname}) payment status is now {req.payment_status}."
    ...
    if admins_accs:
        for user in admins_accs:
            notify_user(
                ...
                message=f"An indent request for {req.quantity} units of {req.item_name} has been raised by {current_user['name']} and is pending approval.",
                type_str="indent"
            )
...
def approve_indent(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    if indent["requested_by"]:
        notify_user(
            ...
            message=f"Your indent request for {indent['quantity']} units of {indent['item_name']} has been approved by {current_user['name']}.",
            type_str="indent"
        )
...
def reject_indent(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    if indent["requested_by"]:
        notify_user(
            ...
            message=f"Your indent request for {indent['quantity']} units of {indent['item_name']} has been rejected by {current_user['name']}.",
            type_str="indent"
        )
...
def create_reminder(req: ReminderCreate, current_user: dict = Depends(get_current_user)):
    ...
    notify_user(
        ...
        message=f"Reminder scheduled: '{req.title}' (Category: {req.category or 'Other'}) for {req.remind_at}.",
        type_str="reminder"
    )
