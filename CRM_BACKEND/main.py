...
# Create notifications and user_devices tables if not exists
try:
...
# Initialize Firebase Admin SDK if credential file is present or env var is set
import json
...
try:
    ...
    if firebase_json:
        ...
        print("Firebase credential file or environment variable not found. Push notifications will be database-only.")
except Exception as e:
    print(f"Error initializing Firebase Admin: {e}")

def send_push_notification(user_id: int, title: str, message: str, type_str: str = "general"):
    if not firebase_initialized:
        return
    try:
        devices = db_query("SELECT fcm_token FROM user_devices WHERE user_id = %s", (user_id,))
        if not devices:
            return
...
import threading
def notify_user(user_id: int, title: str, message: str, type_str: str):
    print(f"DEBUG notify_user: entering for user_id = {user_id}, title = {title}")
    
    # Auto-format notification text to include correct AM/PM timing
    now_ist = get_now_ist()
    time_str = now_ist.strftime('%I:%M %p')
    
    if " AM" not in message and " PM" not in message:
        if message.endswith("."):
            message = message[:-1]
    ...
    try:
        def dispatch_async():
            print(f"DEBUG dispatch_async: started for user_id = {user_id}")
            try:
                # Insert in-app notification in background
                print("DEBUG dispatch_async: inserting notification to DB")
                db_execute(
                    """
                    INSERT INTO notifications (user_id, title, message, type)
                    VALUES (%s, %s, %s, %s)
                    """,
                    (user_id, title, message, type_str)
                )

                # Send push notification to target user
                print("DEBUG dispatch_async: sending push notification to target")
                send_push_notification(user_id, title, message, type_str)
                
                # Send email
                print("DEBUG dispatch_async: querying user details for email")
                user = db_query("SELECT email, role_id FROM users WHERE id = %s", (user_id,), fetch_one=True)
                if user and user["email"]:
                    email_body = f"""
                    <html>
                        <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; margin: 0; padding: 20px;">
                            <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
                                <div style="background: linear-gradient(135deg, #4f46e5, #6366f1); padding: 24px; text-align: center; color: #ffffff;">
                                    <h2 style="margin: 0; font-size: 20px; font-weight: 700; letter-spacing: 0.5px;">Papyrus CRM</h2>
                                </div>
                                <div style="padding: 24px;">
                                    <h3 style="margin-top: 0; color: #1e293b; font-size: 18px; font-weight: 600;">{title}</h3>
                                    <p style="color: #475569; font-size: 15px; margin-bottom: 24px;">{message}</p>
                                    <div style="text-align: center; margin-bottom: 24px;">
                                        <a href="http://localhost:5173/home" style="display: inline-block; background: #4f46e5; color: #ffffff; padding: 12px 24px; font-weight: 600; text-decoration: none; border-radius: 8px; font-size: 14px; box-shadow: 0 2px 4px rgba(79, 70, 229, 0.2);">Open CRM Dashboard</a>
                                    </div>
                    ...
                    print(f"DEBUG dispatch_async: sending email to user_id = {user_id}")
                    send_email(user["email"], title, email_body)

                # Oversight broadcast to ADMIN & HR (if not already ADMIN/HR)
                is_admin_or_hr = False
                if user and user["role_id"]:
                    role_row = db_query("SELECT role_name FROM roles WHERE id = %s", (user["role_id"],), fetch_one=True)
                    if role_row and role_row["role_name"].upper() in ("ADMIN", "HR"):
                        is_admin_or_hr = True

                if not is_admin_or_hr:
                    print("DEBUG dispatch_async: querying admins for oversight")
                    admins = db_query(
                        """
                    ...
                    if admins:
                        for admin in admins:
                            print(f"DEBUG dispatch_async: sending oversight to admin user_id = {admin['id']}")
                            db_execute(
                                """
                                INSERT INTO notifications (user_id, title, message, type)
                                VALUES (%s, %s, %s, %s)
                                """,
                                (admin["id"], f"[Admin Oversight] {title}", message, type_str)
                            )
                            send_push_notification(admin["id"], f"[Admin Oversight] {title}", message, type_str)
                            if admin["email"]:
                                admin_email_body = f"""
                                <html>
                                    <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; margin: 0; padding: 20px;">
                                        <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
                                            <div style="background: linear-gradient(135deg, #ef4444, #f43f5e); padding: 24px; text-align: center; color: #ffffff;">
                                                <h2 style="margin: 0; font-size: 20px; font-weight: 700; letter-spacing: 0.5px;">Papyrus CRM Oversight</h2>
                                            </div>
                                            <div style="padding: 24px;">
                                                <h3 style="margin-top: 0; color: #1e293b; font-size: 18px; font-weight: 600;">Oversight: {title}</h3>
                                                <p style="color: #475569; font-size: 15px; margin-bottom: 24px;">{message}</p>
                                                <div style="text-align: center; margin-bottom: 24px;">
                                                    <a href="http://localhost:5173/home" style="display: inline-block; background: #ef4444; color: #ffffff; padding: 12px 24px; font-weight: 600; text-decoration: none; border-radius: 8px; font-size: 14px; box-shadow: 0 2px 4px rgba(239, 68, 68, 0.2);">Open Admin Dashboard</a>
                                                </div>
                                                <p style="font-size: 12px; color: #94a3b8; border-top: 1px solid #f1f5f9; padding-top: 16px; margin: 0;">
                                ...
                                """
                                from mail import send_email
                                print(f"DEBUG dispatch_async: sending oversight email to admin user_id = {admin['id']}")
                                send_email(admin["email"], f"[Admin Oversight] {title}", admin_email_body)
            except Exception as ex:
                print(f"Error in dispatch_async: {ex}")

        print("DEBUG notify_user: spawning thread")
        threading.Thread(target=dispatch_async, daemon=True).start()
        print("DEBUG notify_user: thread spawned successfully")
    except Exception as e:
...
def login(req: LoginRequest):
    ...
    if not existing:
        ...
        notify_user(
            ...
            message=f"{user['name']} checked in at {now.strftime('%I:%M %p')}.",
            type_str="attendance"
        )
    else:
...
def get_dashboard_stats(current_user: dict = Depends(get_current_user)):
    ...
    my_total_leads = db_query("SELECT COUNT(*) as cnt FROM leads WHERE assigned_sales_id = %s OR created_by = %s", (user_id, user_id), fetch_one=True)["cnt"]
    my_converted_leads = db_query("SELECT COUNT(*) as cnt FROM leads WHERE (assigned_sales_id = %s OR created_by = %s) AND status = 'WON'", (user_id, user_id), fetch_one=True)["cnt"]
    my_active_leads = db_query("SELECT COUNT(*) as cnt FROM leads WHERE (assigned_sales_id = %s OR created_by = %s) AND status = 'HOT'", (user_id, user_id), fetch_one=True)["cnt"]
    my_pipeline_value = db_query("""
        SELECT COALESCE(SUM(o.total_amount), 0) as total 
        FROM orders o 
        LEFT JOIN leads l ON o.lead_id = l.id 
        WHERE (o.created_by = %s OR l.assigned_sales_id = %s OR l.created_by = %s)
          AND (o.lead_id IS NULL OR l.status = 'WON')
    """, (user_id, user_id, user_id), fetch_one=True)["total"]
...
def list_users(current_user: dict = Depends(get_current_user)):
    users = db_query(
        ...
        SELECT u.id, u.employee_id, u.name, u.email, u.phone, r.role_name as role, u.salary, u.is_active,
               u.department, u.designation, u.joining_date, u.biometric_id,
    ...
    for u in users:
        formatted.append({
            ...
            "biometric_id": u["biometric_id"] or "",
...
def get_user(employee_id: str, current_user: dict = Depends(get_current_user)):
    ...
    formatted_user = {
        ...
        "biometric_id": user["biometric_id"] or "",
...
def update_user(employee_id: str, req: UserUpdate, current_user: dict = Depends(RoleChecker(["admin", "hr"]))):
    ...
    db_execute(
        ...
        SET name = %s, email = %s, phone = %s, role_id = %s, salary = %s,
            ...
            biometric_id = %s, modified_by = %s, modified_at = CURRENT_TIMESTAMP
        WHERE id = %s
        """,
        (
            ...
            req.biometric_id, current_user["id"], user["id"]
        )
    )
...
def verify_user(employee_id: str, current_user: dict = Depends(RoleChecker(["admin", "hr"]))):
    ...
    notify_user(
        ...
        message=f"Your account is verified and active.",
...
def check_out(req: LocationUpdate, employee_id: str = Query(...), current_user: dict = Depends(get_current_user)):
    ...
    note_str = f" with task summary: '{req.checkout_note}'" if req.checkout_note else ""
    notify_user(
        user_id=user["id"],
        title="Attendance Checked Out",
        message=f"{user['name']} checked out at {now.strftime('%I:%M %p')}{note_str}.",
        type_str="attendance"
    )
...
# Biometric Device Integration (ZK/ADMS Protocol)
@app.get("/iclock/cdata.aspx")
async def iclock_handshake(request: Request):
    sn = request.query_params.get("SN")
    print(f"Device connected (handshake): SN={sn}")
    return PlainTextResponse("OK")

@app.post("/iclock/cdata.aspx")
async def iclock_data(request: Request):
    body = await request.body()
    raw_data = body.decode(errors="ignore")
    print("Raw biometric device data received:")
    print(raw_data)
    
    # Process the attendance logs line by line
    ...
    for line in lines:
        ...
        if len(parts) >= 3 and "-" in parts[1] and ":" in parts[2]:
            ...
            try:
                ...
                # Fetch user by biometric_id first, then fallback to employee_id, then to numeric id
                user = db_query("SELECT id, name FROM users WHERE biometric_id = %s", (biometric_pin,), fetch_one=True)
                if not user:
                    user = db_query("SELECT id, name FROM users WHERE employee_id = %s", (biometric_pin,), fetch_one=True)
                if not user and biometric_pin.isdigit():
                    user = db_query("SELECT id, name FROM users WHERE id = %s", (int(biometric_pin),), fetch_one=True)
                    
                if user:
                    user_id = user["id"]
                    user_name = user["name"]
                    
                    # Check if there is an active session (check_out IS NULL) for the user on this date
                    att = db_query(
                        """
                    ...
                    if not att or att["check_out"] is not None:
                        # Perform check-in
                        db_execute(
                            "INSERT INTO attendance (user_id, check_in, date) VALUES (%s, %s, %s)",
                            (user_id, log_time, log_date)
                        )
                        notify_user(
                            user_id=user_id,
                            title="Biometric Checked In",
                            message=f"{user_name} checked in via biometric device.",
                            type_str="attendance"
                        )
                        print(f"Biometric Check-In recorded for {user_name} (ID: {user_id}) at {log_time}")
                    else:
                        # Perform check-out on the active session
                        check_in_time = att["check_in"]
                        time_diff = log_time - check_in_time
                        seconds = max(0.0, time_diff.total_seconds())
                        hours = Decimal(seconds / 3600.0)
                        
                        db_execute(
                            "UPDATE attendance SET check_out = %s, work_hours = %s, checkout_note = %s WHERE id = %s",
                            (log_time, hours, "Biometric Check-Out", att["id"])
                        )
                        notify_user(
                            user_id=user_id,
                            title="Biometric Checked Out",
                            message=f"{user_name} checked out via biometric device.",
                            type_str="attendance"
                        )
                        print(f"Biometric Check-Out recorded for {user_name} (ID: {user_id}) at {log_time}")
                else:
                    print(f"Warning: No matching user found for biometric PIN: {biometric_pin}")
            except Exception as parse_ex:
                print(f"Error parsing or saving biometric line '{line}': {parse_ex}")
...
@app.get("/attendance-status/me")
def get_attendance_status(current_user: dict = Depends(get_current_user)):
    today = get_today_ist()
    active = db_query(
        "SELECT check_in FROM attendance WHERE user_id = %s AND date = %s AND check_out IS NULL ORDER BY check_in DESC LIMIT 1",
    ...
    return {
        ...
        "checkin_time": format_dt(active["check_in"]) if active else None
    }
...
def pin_location(req: PinLocationRequest, employee_id: str = Query(...), current_user: dict = Depends(get_current_user)):
    ...
    notify_user(
        ...
        message=f"{user['name']} pinned location: {req.note}.",
        type_str="location"
    )
    ...
    if role == "sales":
        customers = db_query(
            ...
            WHERE c.created_by = %s
               OR l.assigned_sales_id = %s OR l.created_by = %s
        ...
        orders = db_query(
            ...
            WHERE (o.lead_id IS NULL OR l.status = 'WON' OR l.created_by = %s OR o.created_by = %s) AND (
               o.created_by = %s
               OR c.created_by = %s
               OR l.assigned_sales_id = %s OR l.created_by = %s
...
def create_lead(req: LeadCreate, current_user: dict = Depends(get_current_user)):
    print("DEBUG: Entering create_lead")
    uid = current_user["id"]
    ...
    print("DEBUG: customer query done, customer =", customer)
    if not customer and company_val:
        ...
        print("DEBUG: company-based customer query done, customer =", customer)
    ...
    if not expected_delivery_date and req.product_type:
        print("DEBUG: expected_delivery_date is None, querying inventory")
        stock = db_query(
            """
            SELECT i.current_stock
            FROM inventory i 
            JOIN products p ON i.product_id = p.id 
            WHERE LOWER(p.name) = LOWER(%s) AND i.category IN ('Indents', 'Finished Goods')
            """,
            (req.product_type,),
            fetch_one=True
        )
        has_stock = stock and stock["current_stock"] > 0
        days_to_add = 4 if has_stock else 7
        expected_delivery_date = get_today_ist() + timedelta(days=days_to_add)

    print("DEBUG: expected_delivery_date =", expected_delivery_date)
    ...
    cust_info = db_query("SELECT contact_person, company_name FROM customers WHERE id = %s", (cust_id,), fetch_one=True)
    cust_display = f"{cust_info['contact_person']} ({cust_info['company_name']})" if cust_info else "New Customer"

    print("DEBUG: notifying lead creator")
    # Notify creator of the lead
    notify_user(
        user_id=uid,
        title="Lead Created",
        message=f"Lead for {cust_display} created by {current_user['name']}.",
        type_str="lead_creation"
    )

    # Notify assigned sales agent
    if assigned_sales_id:
        notify_user(
            user_id=assigned_sales_id,
            title="New Lead Assigned",
            message=f"Assigned to lead: {cust_display}.",
            type_str="lead_assignment"
        )
        if req.followup_date:
            notify_user(
                user_id=assigned_sales_id,
                title="Lead Follow-up Scheduled",
                message=f"Follow-up for {cust_display} scheduled for {req.followup_date}.",
                type_str="followup"
            )

    # Lead verification request notification for admin/hr
    if db_status.startswith("PENDING_"):
        admins = db_query(
            "SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name IN ('ADMIN', 'HR')"
        )
        if admins:
            for admin in admins:
                notify_user(
                    user_id=admin["id"],
                    title="Lead Verification Request",
                    message=f"Lead for {cust_display} created by {current_user['name']} needs verification.",
                    type_str="lead_verification_request"
                )
...
def update_lead(id: int, req: LeadUpdate, current_user: dict = Depends(get_current_user)):
    ...
    cust_info = db_query("SELECT contact_person, company_name FROM customers WHERE id = %s", (lead["customer_id"],), fetch_one=True)
    cust_display = f"{cust_info['contact_person']} ({cust_info['company_name']})" if cust_info else "Customer"

    if assigned_sales_id and assigned_sales_id != lead["assigned_sales_id"]:
        notify_user(
            user_id=assigned_sales_id,
            title="Lead Reassigned to You",
            message=f"Lead for {cust_display} reassigned to you.",
            type_str="lead_assignment"
        )
        if req.followup_date:
            notify_user(
    ...
    elif assigned_sales_id and req.followup_date and str(req.followup_date) != str(lead["followup_date"]):
        notify_user(
            user_id=assigned_sales_id,
            title="Lead Follow-up Updated",
            message=f"Follow-up for {cust_display} rescheduled to {req.followup_date}.",
            type_str="followup"
        )
...
def verify_lead(id: int, current_user: dict = Depends(RoleChecker(["admin", "hr"]))):
    lead = db_query("SELECT status, assigned_sales_id, customer_id FROM leads WHERE id = %s", (id,), fetch_one=True)
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found")
    ...
    if lead.get("assigned_sales_id"):
        cust_info = db_query("SELECT contact_person, company_name FROM customers WHERE id = %s", (lead["customer_id"],), fetch_one=True)
        cust_display = f"{cust_info['contact_person']} ({cust_info['company_name']})" if cust_info else "Customer"
        notify_user(
            user_id=lead["assigned_sales_id"],
            title="Lead Verified by Admin",
            message=f"Lead for {cust_display} approved and active ({new_status}).",
            type_str="lead_verification"
        )
...
def deduct_and_lock_inventory(cur, order_id: int, customer_id: int, product_name: str, quantity: float):
    ...
    if prod_row:
        ...
        cur.execute("INSERT INTO products (name, unit) VALUES (%s, 'Units') RETURNING id", (product_name,))
    ...
    cur.execute(
        ...
        WHERE product_id = %s AND category IN ('Indents', 'Finished Goods')
        """,
        (product_id,)
    )
    ...
    # 3. Insert record in locked_inventory
    cur.execute(
        """
        INSERT INTO locked_inventory (order_id, customer_id, inventory_id, quantity)
        VALUES (%s, %s, %s, %s)
        """,
        (order_id, customer_id, inventory_id, Decimal(str(quantity)))
    )

    if is_in_stock:
        cur.execute("UPDATE orders SET in_stock = TRUE WHERE id = %s", (order_id,))

    # 4. Check stock alert
    cur.execute("SELECT current_stock, minimum_stock FROM inventory WHERE id = %s", (inventory_id,))
    inv_check = cur.fetchone()
    if inv_check:
        curr_stock, min_stock = inv_check
        if curr_stock <= min_stock:
            now_ist = get_now_ist()
            time_str = now_ist.strftime('%I:%M %p')
            msg = f"Low stock: {product_name} is {curr_stock} (Min: {min_stock}) at {time_str}."
            cur.execute(
                """
                SELECT u.id, u.email FROM users u 
                JOIN roles r ON u.role_id = r.id 
            ...
            for u_id, u_email in users_to_notify:
                cur.execute(
                    "INSERT INTO notifications (user_id, title, message, type) VALUES (%s, %s, %s, %s)",
                    (u_id, f"Low Stock Alert [{time_str}]", msg, "stock")
                )
                if u_email:
                    email_body = f"""
                    <html>
                        <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; margin: 0; padding: 20px;">
                            <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
                                <div style="background: linear-gradient(135deg, #ef4444, #f87171); padding: 24px; text-align: center; color: #ffffff;">
                                    <h2 style="margin: 0; font-size: 20px; font-weight: 700; letter-spacing: 0.5px;">Papyrus CRM Stock Alert</h2>
                                </div>
                                <div style="padding: 24px;">
                                    <h3 style="margin-top: 0; color: #1e293b; font-size: 18px; font-weight: 600;">Low Stock Alert</h3>
                                    <p style="color: #475569; font-size: 15px; margin-bottom: 24px;">{msg}</p>
                                    <div style="text-align: center; margin-bottom: 24px;">
                                        <a href="http://localhost:5173/inventory" style="display: inline-block; background: #ef4444; color: #ffffff; padding: 12px 24px; font-weight: 600; text-decoration: none; border-radius: 8px; font-size: 14px;">View Inventory</a>
                                    </div>
                                </div>
                             </div>
                        </body>
                    ...
                    """
                    from mail import send_email
                    threading.Thread(target=send_email, args=(u_email, "Low Stock Alert", email_body), daemon=True).start()
...
def convert_lead_to_deal(id: int, req: LeadConvertRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Notify sales agent and admins about lead conversion to order
    cust_display = f"{lead['contact_person']} ({lead['company_name']})"
    msg = f"Lead for {cust_display} converted to Order {order_number} (₹{total_val})."
    
    if lead["assigned_sales_id"]:
        notify_user(lead["assigned_sales_id"], "Lead Converted to Order", msg, "order_status")
        
    admins = db_query("SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name = 'ADMIN'")
    for admin in admins:
        if admin["id"] != lead["assigned_sales_id"]:
            notify_user(admin["id"], "Lead Converted to Order", msg, "order_status")
...
def list_deals(current_user: dict = Depends(get_current_user)):
    ...
    query = """
        SELECT o.id, o.customer_id, o.lead_id, o.order_number, o.product_type, o.size, o.color, o.gsm,
               ...
               u3.employee_id as order_creator_employee_id,
               ...
               o.inventory_status, o.source, o.priority, o.followup_date
        FROM orders o
    ...
    if role == "sales":
        query += """
            AND (o.created_by = %s OR l.assigned_sales_id = %s OR l.created_by = %s
               OR o.customer_id IN (
        ...
        orders = db_query(query, (user_id, user_id, user_id, user_id, user_id))
    else:
        query += " ORDER BY o.created_at DESC"
    ...
    for o in orders:
        ...
        created_by = o["creator_employee_id"] or o["order_creator_employee_id"] or current_user["employee_id"]
        ...
        formatted.append({
            ...
            "followup_date": format_dt(o["followup_date"])
        })
...
def create_deal(req: DealCreate, current_user: dict = Depends(get_current_user)):
        ...
        with pool.connection() as conn:
            with conn:
                with conn.cursor() as cur:
                    ...
                    if phone_val or email_val:
                        if phone_val and email_val:
                            cur.execute("SELECT id FROM customers WHERE (phone = %s AND phone != '') OR (email = %s AND email != '') LIMIT 1", (phone_val, email_val))
                        elif phone_val:
                            cur.execute("SELECT id FROM customers WHERE phone = %s AND phone != '' LIMIT 1", (phone_val,))
                        elif email_val:
                            cur.execute("SELECT id FROM customers WHERE email = %s AND email != '' LIMIT 1", (email_val,))
                        cust_row = cur.fetchone()
                        if cust_row:
                            customer = {"id": cust_row[0]}

                    if not customer and company_val:
                        cur.execute("SELECT id FROM customers WHERE company_name = %s AND company_name != '' LIMIT 1", (company_val,))
                        cust_row = cur.fetchone()
                        if cust_row:
                            customer = {"id": cust_row[0]}
                    ...
                    if customer:
                        ...
                        cur.execute(
                            "UPDATE customers SET company_name = %s, contact_person = %s, address = %s, modified_by = %s, modified_at = CURRENT_TIMESTAMP WHERE id = %s",
                            (req.company_name, req.contact_name, req.location, uid, cust_id)
                        )
                    else:
                        cur.execute(
                            """
                            ...
                            VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id
                            """,
                            (req.company_name, req.contact_name, req.phone, req.email, req.location, uid, uid)
                        )
                    ...
                    expected_deliv = req.expected_delivery_date
                    if not expected_deliv:
                        product_type = req.product_type or "Standard Box"
                        cur.execute(
                            """
                            SELECT i.current_stock
                            FROM inventory i 
                        ...
                        has_stock = stock_row and stock_row[0] > 0
                        days_to_add = 4 if has_stock else 7
                        expected_deliv = get_today_ist() + timedelta(days=days_to_add)
    ...
    # Notify creator and admins about order creation
    cname = req.company_name or "Customer"
    msg = f"Order {order_number} created for {cname} (₹{total_val})."
    
    notify_user(uid, "New Order Created", msg, "order_status")
    
    admins = db_query("SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name = 'ADMIN'")
    for admin in admins:
        if admin["id"] != uid:
            notify_user(admin["id"], "New Order Created", msg, "order_status")
...
def update_deal(id: int, req: DealUpdate, stage: Optional[str] = None, status: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    ...
    if role == "sales":
        if order["created_by"] != uid:
            lead = db_query("SELECT * FROM leads WHERE id = %s", (order["lead_id"],), fetch_one=True) if order["lead_id"] else None
    ...
    quantity = req.quantity if req.quantity is not None else order["quantity"]
    unit_price = req.unit_price if req.unit_price is not None else order["unit_price"]

    if req.deal_value and req.deal_value > 0:
        total_val = req.deal_value
    elif quantity is not None and unit_price is not None:
        total_val = Decimal(str(quantity)) * Decimal(str(unit_price))
    else:
        total_val = order["total_amount"] or Decimal("0")

    customer = db_query("SELECT * FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
    if not customer:
        customer = {"company_name": "", "contact_person": "", "phone": "", "email": "", "address": ""}

    expected_deliv = req.expected_delivery_date
    if not expected_deliv and req.product_type:
        stock = db_query(
            """
            SELECT i.current_stock
            FROM inventory i 
        ...
        days_to_add = 4 if has_stock else 7
        expected_deliv = get_today_ist() + timedelta(days=days_to_add)
    elif not expected_deliv:
        expected_deliv = order["expected_delivery_date"]
    ...
    db_execute(
        ...
        (
            ...
            expected_deliv,
            req.source if req.source is not None else order["source"],
            req.priority if req.priority is not None else order["priority"],
            req.followup_date if req.followup_date is not None else order["followup_date"],
    ...
    db_execute(
        ...
        (
            ...
            req.location if req.location is not None else customer["address"],
            uid,
            order["customer_id"]
        )
    )
    ...
    # Notify sales rep and admins if order status has updated
    if db_status != order["status"]:
        cname = customer["company_name"] or "Customer"
        msg = f"Order {order['order_number']} ({cname}) status: {final_stage} | {final_status}."
        
        # Notify creator
        notify_user(order["created_by"], "Order Status Updated", msg, "order_status")
        
        # Notify lead assignee if exists
        if order["lead_id"]:
            lead = db_query("SELECT assigned_sales_id FROM leads WHERE id = %s", (order["lead_id"],), fetch_one=True)
            if lead and lead["assigned_sales_id"] and lead["assigned_sales_id"] != order["created_by"]:
                notify_user(lead["assigned_sales_id"], "Order Status Updated", msg, "order_status")
                
        # Notify admins
        admins = db_query("SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name = 'ADMIN'")
        for admin in admins:
            if admin["id"] != order["created_by"]:
                notify_user(admin["id"], "Order Status Updated", msg, "order_status")
...
def approve_deal(id: int, current_user: dict = Depends(RoleChecker(["admin"]))):
    ...
    # Notify sales rep and lead assignee
    cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
    cname = cust["company_name"] if cust else "Customer"
    msg = f"Order {order['order_number']} ({cname}) approved by admin."
    notify_user(order["created_by"], "Order Approved", msg, "order_status")
    if order["lead_id"]:
        lead = db_query("SELECT assigned_sales_id FROM leads WHERE id = %s", (order["lead_id"],), fetch_one=True)
        if lead and lead["assigned_sales_id"] and lead["assigned_sales_id"] != order["created_by"]:
            notify_user(lead["assigned_sales_id"], "Order Approved", msg, "order_status")
...
def reject_deal(id: int, current_user: dict = Depends(RoleChecker(["admin"]))):
    ...
    cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
    cname = cust["company_name"] if cust else "Customer"
    msg = f"Order {order['order_number']} ({cname}) rejected by admin."
    notify_user(order["created_by"], "Order Rejected", msg, "order_status")
    if order["lead_id"]:
        lead = db_query("SELECT assigned_sales_id FROM leads WHERE id = %s", (order["lead_id"],), fetch_one=True)
        if lead and lead["assigned_sales_id"] and lead["assigned_sales_id"] != order["created_by"]:
            notify_user(lead["assigned_sales_id"], "Order Rejected", msg, "order_status")
...
@app.get("/locked-inventory")
def get_locked_inventory(current_user: dict = Depends(get_current_user)):
    records = db_query(
        """
        SELECT li.id, li.quantity, li.locked_at,
               o.order_number, c.company_name, c.contact_person,
    ...
    for r in records:
        formatted.append({
            ...
            "locked_at": format_dt(r["locked_at"]),
...
def update_inventory_item(id: int, req: InventoryItemUpdate, current_user: dict = Depends(RoleChecker(["admin", "inventory"]))):
    ...
    # Check if stock fell below minimum stock
    if req.current_stock <= req.minimum_stock:
        msg = f"Low stock: {req.item_name} is {req.current_stock} (Min: {req.minimum_stock})."
        users_to_notify = db_query(
            """
        ...
        for u in users_to_notify:
            notify_user(u["id"], "Low Stock Alert", msg, "stock")
...
def raise_purchase_request(req: PurchaseRequestCreate, current_user: dict = Depends(get_current_user)):
    ...
    # Notify admins and accountants about the new purchase request
    admins_accs = db_query(
        "SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name IN ('ADMIN', 'ACCOUNTANT', 'ACCOUNTS')"
    )
    if admins_accs:
        for user in admins_accs:
            notify_user(
                user_id=user["id"],
                title="Purchase Request Raised",
                message=f"PR for {req.quantity} units of {req.item_name} raised by {current_user['name']}.",
                type_str="purchase_request"
            )
...
def approve_purchase_request(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    notify_user(
        ...
        message=f"PR for {pr['quantity']} units of {pr['item_name']} approved by {current_user['name']}.",
        type_str="purchase_request"
    )
...
def reject_purchase_request(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    notify_user(
        ...
        message=f"PR for {pr['quantity']} units of {pr['item_name']} rejected by {current_user['name']}.",
        type_str="purchase_request"
    )
...
def list_production_records(current_user: dict = Depends(get_current_user)):
    records = db_query(
        """
        SELECT p.*, o.order_number, o.product_type, o.quantity, o.size, o.color, o.gsm, o.handles, o.print_color, o.bag_type, o.in_stock, c.company_name
        FROM production p
...
def update_production_status(id: int, req: ProductionUpdate, current_user: dict = Depends(RoleChecker(["admin", "production"]))):
    ...
    # Notify sales agent and admins of production status update
    order = db_query("SELECT order_number, created_by, lead_id, customer_id FROM orders WHERE id = %s", (prod["order_id"],), fetch_one=True)
    if order:
        cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
        cname = cust["company_name"] if cust else "Customer"
        msg = f"Production for Order {order['order_number']} ({cname}) is {req.status}."
        
        # Notify sales rep
        notify_user(order["created_by"], "Production Status Updated", msg, "order_status")
        
        # Notify lead assignee
        if order["lead_id"]:
            lead = db_query("SELECT assigned_sales_id FROM leads WHERE id = %s", (order["lead_id"],), fetch_one=True)
            if lead and lead["assigned_sales_id"] and lead["assigned_sales_id"] != order["created_by"]:
                notify_user(lead["assigned_sales_id"], "Production Status Updated", msg, "order_status")
                
        # Notify admins
        admins = db_query("SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name = 'ADMIN'")
        for admin in admins:
            if admin["id"] != order["created_by"]:
                notify_user(admin["id"], "Production Status Updated", msg, "order_status")

    if req.status == "COMPLETED":

        inv = db_query("SELECT id FROM invoices WHERE order_id = %s", (prod["order_id"],), fetch_one=True)
        if not inv:
            order_data = db_query("SELECT * FROM orders WHERE id = %s", (prod["order_id"],), fetch_one=True)
...
def update_invoice_payment_status(id: int, req: InvoicePaymentUpdate, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    # Notify sales rep and admins of payment status update
    order = db_query("SELECT order_number, created_by, lead_id, customer_id FROM orders WHERE id = %s", (inv["order_id"],), fetch_one=True)
    if order:
        cust = db_query("SELECT company_name FROM customers WHERE id = %s", (order["customer_id"],), fetch_one=True)
        cname = cust["company_name"] if cust else "Customer"
        msg = f"Invoice {inv['invoice_number']} ({cname}) payment: {req.payment_status}."
        
        # Notify sales rep
        notify_user(order["created_by"], "Payment Status Updated", msg, "order_status")
        
        # Notify lead assignee
        if order["lead_id"]:
            lead = db_query("SELECT assigned_sales_id FROM leads WHERE id = %s", (order["lead_id"],), fetch_one=True)
            if lead and lead["assigned_sales_id"] and lead["assigned_sales_id"] != order["created_by"]:
                notify_user(lead["assigned_sales_id"], "Payment Status Updated", msg, "order_status")
                
        # Notify admins
        admins = db_query("SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name = 'ADMIN'")
        for admin in admins:
            if admin["id"] != order["created_by"]:
                notify_user(admin["id"], "Payment Status Updated", msg, "order_status")
...
def create_customer(req: CustomerCreate, current_user: dict = Depends(get_current_user)):
    ...
    cust_id = db_execute(
        """
        INSERT INTO customers (company_name, contact_person, phone, alternate_phone, email, address, gst_number, notes, created_by, modified_by)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id
        """,
        (req.company_name, req.contact_person, req.phone, req.alternate_phone, req.email, req.address, req.gst_number, req.notes, current_user["id"], current_user["id"]),
        return_id=True
    )
...
def create_indent(req: IndentCreate, current_user: dict = Depends(get_current_user)):
    ...
    # Notify admins/accountants about the new indent request
    admins_accs = db_query(
        "SELECT u.id FROM users u JOIN roles r ON u.role_id = r.id WHERE r.role_name IN ('ADMIN', 'ACCOUNTANT', 'ACCOUNTS')"
    )
    if admins_accs:
        for user in admins_accs:
            notify_user(
                user_id=user["id"],
                title="Indent Request Raised",
                message=f"Indent for {req.quantity} units of {req.item_name} raised by {current_user['name']}.",
                type_str="indent"
            )
...
def approve_indent(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    if indent["requested_by"]:
        notify_user(
            ...
            message=f"Indent for {indent['quantity']} units of {indent['item_name']} approved by {current_user['name']}.",
            type_str="indent"
        )
...
def reject_indent(id: int, current_user: dict = Depends(RoleChecker(["admin", "accountant", "accounts"]))):
    ...
    if indent["requested_by"]:
        notify_user(
            ...
            message=f"Indent for {indent['quantity']} units of {indent['item_name']} rejected by {current_user['name']}.",
            type_str="indent"
        )
...
def get_alerts(current_user: dict = Depends(get_current_user)):
    ...
    reminders = db_query("""
        SELECT id, title, description, remind_at, category
        FROM reminders
...
def create_reminder(req: ReminderCreate, current_user: dict = Depends(get_current_user)):
    reminder_id = db_execute(
        """
        INSERT INTO reminders (user_id, title, description, remind_at, category)
        VALUES (%s, %s, %s, %s, %s) RETURNING id
        """,
        (current_user["id"], req.title, req.description, req.remind_at, req.category or "Other"),
        return_id=True
    )
    ...
    notify_user(
        ...
        message=f"Reminder: '{req.title}' ({req.category or 'Other'}) for {req.remind_at}.",
        type_str="reminder"
    )

    return {"status": "success", "message": "Reminder created successfully", "reminder_id": reminder_id}

@app.get("/locked_inventory")
def list_locked_inventory(current_user: dict = Depends(get_current_user)):
    records = db_query("""
        SELECT li.id, li.quantity, li.locked_at, 
               o.order_number, o.product_type,
...
def read_notification(id: int, current_user: dict = Depends(get_current_user)):
    user_id = current_user["id"]
    # Check if notification exists and belongs to user
    notif = db_query("SELECT id FROM notifications WHERE id = %s AND user_id = %s", (id, user_id), fetch_one=True)
    if not notif:
        raise HTTPException(status_code=404, detail="Notification not found")
    
    db_execute("UPDATE notifications SET is_read = TRUE WHERE id = %s", (id,))
    return {"status": "success", "message": "Notification marked as read"}

@app.delete("/notifications/clear")
def clear_notifications(current_user: dict = Depends(get_current_user)):
    user_id = current_user["id"]
    db_execute("DELETE FROM notifications WHERE user_id = %s", (user_id,))
...
def register_device(req: RegisterDeviceRequest, current_user: dict = Depends(get_current_user)):
    try:
        db_execute(
            """
            ...
            """,
            (current_user["id"], req.fcm_token, req.device_type)
        )
